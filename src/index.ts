...
export class GitIngestMCP extends McpAgent<Env> {
  ...
  async init() {
    ...
    this.server.tool(
      ...
      {
        repo_url: z.string().describe("GitHub repository URL (e.g., https://github.com/user/repo)"),
        include_patterns: z.array(z.string()).optional().describe("Unix shell-style patterns to include (e.g., ['*.py', '*.js'])"),
        exclude_patterns: z.array(z.string()).optional().describe("Unix shell-style patterns to exclude (e.g., ['node_modules/*', '*.log'])"),
        max_file_size: z.number().optional().describe("Maximum file size in bytes to process")
      },
      async ({ repo_url, include_patterns, exclude_patterns, max_file_size }) => {
        try {
    ...
    this.server.tool(
      ...
      {
        repo_url: z.string().describe("GitHub repository URL"),
        language: z.enum(["python", "javascript", "typescript", "go", "rust", "java", "csharp"])
